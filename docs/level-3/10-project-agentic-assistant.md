...
**Why budget enforcement has to sit structurally outside the agent's own
reasoning, not inside its prompt.** Lesson 1 established that an agentic
loop's "keep searching or stop" decision is produced by the same next-token
prediction mechanism as any other generation — which means it inherits no
built-in guarantee of termination, no matter how the prompt is worded.
Budget enforcement (a hard cap on hop count, tool calls, or token spend
checked by code *outside* the model's generation loop) is a control
mechanism the model cannot talk its way around, precisely because it isn't
implemented as an instruction the model has to choose to follow — it's a
loop condition the orchestrating code evaluates independently of what the
model generates. This is the same principle as level-2 lesson 5's point
about metadata as a security boundary: enforcement one layer below the
model's own reasoning survives cases where the model's in-context judgment
fails or gets talked past.

**Why this project's building blocks compose without needing to be
redesigned for each other.** The retriever-plus-cache block (lesson 7's
caching mechanism), the budget enforcer (lesson 1's loop risk, addressed
structurally), and entity extraction for hop-chaining (lesson 2's mechanism
for keeping each hop's query single-topic) all operate on the same seam
identified back in level-1 lesson 7: they each sit between "embed" and
"retrieve," or between one hop's answer and the next hop's query
construction. None of them change the fundamental two-phase, embed-then-
search shape of RAG — they compose because each one is a well-scoped
addition to a single seam in an unchanged pipeline, which is exactly the
property that let Level 1 through Level 3 build on the same mental model
one mechanism at a time instead of each lesson requiring a new
architecture.
