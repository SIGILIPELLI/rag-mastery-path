...
**Why benchmarking on your own data is not optional the way it sounds.**
Public embedding leaderboards (MTEB and similar) rank models by average
performance across dozens of unrelated benchmark datasets — legal text,
Reddit comments, scientific papers — and that average tells you almost
nothing about how a model's contrastive training distribution overlaps with
*your* domain's vocabulary and notion of relevance (lesson 2, level-1's
point about what makes a space "semantic": it's shaped entirely by the
training pairs the model saw). A model that ranks 3rd on MTEB but was never
exposed to anything resembling your internal jargon can be measurably worse
on your queries than a model that ranks 15th but happens to share
vocabulary with your domain — which is why this lesson's benchmark against
your own (query, correct-document) pairs and hit-rate/MRR (level-1 lesson 8)
is the only score that actually predicts your system's retrieval quality.

**Why dimension is a direct, computable trade against both cost and
recall.** Every stored vector costs `dimension × 4 bytes` (float32) in the
index — level-3 lesson 4's memory wall scales linearly with dimension, so
doubling dimension roughly doubles both storage and the per-comparison
distance computation cost. Higher-dimensional embeddings *can* encode more
distinguishing information (more room for the contrastive training to
separate fine-grained distinctions), but past a model-specific point,
added dimensions mostly encode noise or redundant signal rather than new
semantic resolution — which is why picking embedding dimension isn't "bigger
is strictly better," it's a real operating point on a cost/recall curve that
your own benchmark (not a vendor's marketing number) should determine.

**Why fine-tuning changes the embedding space itself, not just the
retrieval pipeline around it.** Switching embedding models picks a
*different, already-fixed* space to work in — same idea as choosing a
pre-trained model outright. Fine-tuning instead continues contrastive
training (the same InfoNCE-style loss from level-1 lesson 2) on
*your own* (query, relevant-document) pairs, nudging the existing space so
your domain's specific notion of relevance is pulled closer together than
generic contrastive pretraining achieved — worthwhile once you have enough
real query-document pairs (typically thousands) to move the space
meaningfully without overfitting to a small, unrepresentative sample, and
not worthwhile before then, since fine-tuning on too few or non-
representative pairs just reshapes the space around noise instead of signal.

**Why eval-set contamination silently invalidates every number above.**
If your benchmark's (query, correct-doc) pairs were themselves generated by
an LLM using the same documents the embedding model will retrieve from — or
worse, if the embedding model's own base training corpus overlapped with
your eval documents — the reported hit-rate/MRR measures memorization or
leaked correlation, not generalizable retrieval quality, which is why a
